...
<%
    ...
    System.out.println("st_dailyUpdateDB.jsp 실행 (by login_id : " + login_id + ")");

    if (!DBUtil.assureStudent(login_id) || login_id.equals("")) {
        System.out.println("st_dailyUpdateDB.jsp : 학생 외 us_id의 접근 제한 => login.html로 이동");
...
<%
        try {
            System.out.print("st_dailyUpdateDB.jsp에서 호출한 ");
            ...
            if (todayStr.equals(lastUpdatedDayStr)) { // 오늘 날짜가 최종 업데이트 날짜인 경우
                System.out.println("오늘 날짜와 최종 업데이트 날짜가 동일하니까, reser_stu_main.jsp로 가자");
                ...
                // 1. 우선 최종 업데이트 날짜를 보관하는 파일을 갱신한다.
                // 파일 객체 생성
                ...
                while (rsGetPossible.next()) {
                    ...
                    System.out.println(ca_name + ", " + it_serial + "의 it_status = '0' (노쇼)로 갱신함" );
                }
